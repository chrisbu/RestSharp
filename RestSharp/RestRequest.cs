...
namespace RestSharp
{
  ...
  public class RestRequest : IRestRequest
  {
    ...
    public override string ToString()
    {
      ...
      sb.AppendLine("RestRequest -- start");

      sb.AppendLine(String.Format("Resource={0}", this.Resource));

      sb.AppendLine("Parameters:");
      foreach (var param in this.Parameters)
      {
        sb.Append(String.Format("[{0}:{1}={2}] ", param.Type, param.Name, param.Value));
      }
      sb.AppendLine("");

      //sb.AppendLine(String.Format(this.));
      sb.AppendLine(String.Format("Timeout={0}", this.Timeout));
      sb.AppendLine(String.Format("Attempts={0}", this.Attempts));

      sb.AppendLine("RestRequest -- end");
