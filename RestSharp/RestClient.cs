...
namespace RestSharp
{
  ...
  {
    Logger logger = LogManager.GetLogger("RestSharp.RestClient");
    ...
    private string EncodeParameters(IRestRequest request)
    {
      ...
      logger.Debug("EncodeParameters: querystring= {0}", querystring);
      return querystring.ToString();
    }

    private void ConfigureHttp(IRestRequest request, IHttp http)
    {
      logger.Trace("ConfigureHttp -- Start");
      ...
      foreach (var header in headers)
      {
        ...
        logger.Trace("Adding header: {0}={1}", header.Name, header.Value);
      }
      ...
      foreach (var parameter in @params)
      {
        ...
        logger.Trace("Adding parameter: {0}={1}", parameter.Name, parameter.Value);
      }
      ...
      if (body != null)
      {
        ...
        logger.Trace("RequestContentType = {0}", http.RequestContentType);
        logger.Trace("RequestBody = {0}", http.RequestBody);
      }
      logger.Trace("ConfigureHttp -- End");
    }
    ...
    private RestResponse ConvertToRestResponse(IRestRequest request, HttpResponse httpResponse)
    {
      ...
      {
        logger.Trace("ConvertToRestResponse: RestResponse={0}", restResponse.ToString());
      }
      return restResponse;
    }
    ...
    private IRestResponse<T> Deserialize<T>(IRestRequest request, IRestResponse raw)
    {
      if (logger.IsTraceEnabled)
      {
        logger.Trace("Deserialize: Request: {0}", request.ToString());
      }
      ...
      try
      {
        ...
        if (logger.IsTraceEnabled)
        {
          logger.Trace("Deserialize: Response: {0}", response.ToString());
        }
      ...
      {
        logger.ErrorException(String.Format("RestClient Deserialize<T>: {0}\nResponse:{1}", ex.Message, response.ToString()), ex);
